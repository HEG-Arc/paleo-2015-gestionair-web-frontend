...
angular.module('gestionairApp')
  .directive('slideshow', function ($timeout) {
    return {
      template: '<div class="slideshow"flex><img ng-src="{{images[activeIndex]}}"/><div class="fade" layout-fill ng-repeat="src in images" ng-show="$index == activeIndex" style="background-image: url(\'{{src}}\')"></div></div>',
      replace: true,
      restrict: 'E',
      scope: {
        images: '=',
